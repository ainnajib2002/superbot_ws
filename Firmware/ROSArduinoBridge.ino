...
#define USE_LOADCELL        // Aktifkan pengukuran timbangan
...
#include "sensors.h"          // Fungsi sensor
  ...
  #include "diff_controller.h"   // Fungsi pengendali PID
...
int runCommand() {
...
#ifdef USE_LOADCELL
  case READ_LOADCELL:
    Serial.println(readLoadcell(), 2);  // Tampilkan 2 desimal
    break;
  ...
  case SET_CALIBRATION:
    setCalibrationFactor(atof(argv1));  // Pakai fungsi yang benar
    Serial.print("New Cal Factor: ");
    Serial.println(atof(argv1), 6);     // Tampilkan 6 digit desimal
    break;

  case READ_RAWLOADCELL:
    Serial.print("RAW: ");
    Serial.println(readRawLoadcell());
    break;
#endif
...
#ifdef USE_LOADCELL  //fungsi loadcell
  setup_loadcell();
#endif
}
